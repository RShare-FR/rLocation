...
RMenu.Add('RShare_location', 'main', RageUI.CreateMenu("Location", "", 10,222))
RMenu:Get('RShare_location', 'main'):SetSubtitle("~b~Choisir votre véhicule :")
RMenu:Get('RShare_location', 'main').EnableMouse = false
RMenu:Get('RShare_location', 'main').Closed = function()
    IsMenuOpen = false
    DeleteEntity(Entity)
...
local location = {
    {
        pos = vector3(401.80, -380.99, 46.86),
        sortie = {
            {pos = vector3(395.84, -385.18, 46.62), heading = 108.2},
            {pos = vector3(401.82, -382.93, 46.73), heading = 110.6},
        }
    },
}
    ...
    while true do
        ...
        for k,v in pairs(location) do
            ...
            if dst <= 2.0 then
                ...
                ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour louer un véhicule.")
...
function openLocation()
    if not IsMenuOpen then
        ...
        IsMenuOpen = true 
        RageUI.Visible(RMenu:Get('RShare_location', 'main'), true)

            Citizen.CreateThread(function()
                while IsMenuOpen do
                    Citizen.Wait(1)
                    RageUI.IsVisible(RMenu:Get('RShare_location', 'main'), true, true, true, function()
                        for k,v in pairs(vehs) do
                            RageUI.ButtonWithStyle(v.nom, nil ,{RightLabel = ">"}, true, function(Hovered, Active, Selected) 
                                if (Active) then
                                    if localVeh ~= GetHashKey(v.spawn) then
                                        ...
                                        local veh = CreateVehicle(GetHashKey(v.spawn), zone, heading, 0, 0) -- cahngé le 1
                                        Entity = veh
                            ...
                            if (Selected) then 
                                ...
                                if found then
                                    ...
                                    RageUI.CloseAll()
                                    IsMenuOpen = false
                                    print('^3[RShare] --> https://discord.gg/ysvPBAFwxD')
                                end
                            end
                        end)
                    end
